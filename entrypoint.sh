...
if [ "${IS_LOCAL}" = "true" ]; then echo "reseting db and reseeding" && npx knex migrate:rollback && npx knex migrate:latest && npx knex seed:run ; else echo "applying migrations prod" && npx knex migrate:rollback && npx knex migrate:latest && npx knex seed:run ; fi
